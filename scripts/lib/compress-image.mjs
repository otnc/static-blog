...
// 画像圧縮の共通ロジック。
// `pnpm file` のアップロード (compress-image → compressUpload) と `pnpm comp` / `pnpm comp:all` / `pnpm force-comp` (recompress) で共有する。

// 方針: サイズを削減する。圧縮の強さは 2 段階のプリセットで切り替える。
//  - normal : 画質をあまり落とさず削減する標準モード。アップロード保存時・編集保存時・`pnpm comp` で使用する。
//  - force  : 画質を一段階落としてでも大幅にサイズを削減する強圧縮モード。`pnpm force-comp` で使用する。大きい画像は長辺を縮小し、静止 WebP も再エンコード対象に含める。

// 拡張子について:
//  - アップロード (compressUpload): 静止画はすべて PNG に変換する。新規ファイルなので拡張子が変わっても記事参照に影響しない。アニメ (GIF / APNG / animated WebP) だけは再エンコードせず元形式を保持する。
//  - 再圧縮 (recompress): 拡張子を変えると記事内の ![](...) 参照が壊れるため、同じ拡張子で書き戻す (PNG / JPEG、force では静止 WebP も対象)。
...
// normal は画質・色の劣化を抑えつつサイズを削る標準 (アップロード保存・編集保存・pnpm comp)。
...
// プリセットに maxDimension が設定されていれば、長辺がそれを超える画像をアスペクト比を保ったまま縮小する (拡大はしない)。
...
function encodeJpeg(buffer, p) {
  return pipeline(buffer, p)
    .jpeg({
      ...
      mozjpeg: true,
      // 4:2:0 だと色がにじむので 4:4:4 で色情報を保持する
...
/**
 * 既存ファイルの再圧縮 (拡張子は保持)。PNG / JPEG / 静止 WebP が対象で、プリセットに応じて大きい画像は縮小する。force ほど画質を落とす。
